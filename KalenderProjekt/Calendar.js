        ...
        fetch("http://dhbw.radicalsimplicity.com/calendar/" + iduser + "/events", {
            ...
            let eventsArray = JSON.parse(text);
                ...
                for(let i = 0; i<eventsArray.length; i++){
                    document.getElementById("list").insertAdjacentHTML("beforeend", i+1 + ".Event</br>");
                    items = "ID: " + `${eventsArray[i].id}</br>`;
                    console.log(items);
                    document.getElementById("list").insertAdjacentHTML("beforeend", items);
                    items = "Title: " + `${eventsArray[i].title}</br>`;
                    document.getElementById("list").insertAdjacentHTML("beforeend", items);
                    items = "Location: " + `${eventsArray[i].location}</br>`;
                    document.getElementById("list").insertAdjacentHTML("beforeend", items);
                    items = "Start: " + `${eventsArray[i].start}</br>`;
                    document.getElementById("list").insertAdjacentHTML("beforeend", items);
                    items = "End: " + `${eventsArray[i].end}</br>`;
                    document.getElementById("list").insertAdjacentHTML("beforeend", items);
                    items = "Organizer: " + `${eventsArray[i].organizer}</br>`;
                    document.getElementById("list").insertAdjacentHTML("beforeend", items);
                    items = "Webpage: " + `${eventsArray[i].webpage}</br>`;
                    document.getElementById("list").insertAdjacentHTML("beforeend", items);
                    items = "Allday: " + `${eventsArray[i].allday}</br>`;
                    if(eventsArray[i].allday == true){
                        items = "Allday: " + `Ja</br>`;
                    } else{
                        items = "Allday: " + `Nein</br>`;
                    }
                    document.getElementById("list").insertAdjacentHTML("beforeend", items);
   ...
   /* login() {
    ...
    clickSlot(hour, dayIndex) {
        ...
        const event = new Event({
            ...
            description: "",
            color: "red",
            organizer: "cool@l.com",
            status: "",
            allday: "",
            webpage: "https://www.haltdochdeinmaul.com",
        });
